...
def get_classes(data,Start,End):
    ...
    for idx, end_pt in enumerate(clog_start_pts):
        ...
        if (start_pt > 0) & (start_pt > clog_end_pts[idx]):
            ...
            preclog_T.append(t_span[start_pt:end_pt])
...
if __name__ == '__main__':
    ...
    Start = 0
    End = 710000
    class_1, class_2,preclog_T,noclog_T = get_classes(clean_data, Start, End)

    for i in range(len(class_1)):
        mean = np.mean(class_1[i])
        difference = class_1[i]-mean
        diffdifference = np.cumsum(difference)
        mean2 = np.mean(class_2[i])
        difference2 = class_2[i]-mean2
        diffdifference2 = np.cumsum(difference2)
        plt.plot(preclog_T[i],diffdifference2)
        plt.show()
